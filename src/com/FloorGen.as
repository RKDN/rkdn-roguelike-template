		...
		public function PartitionGen():void
		{
			...
			for (var i:int = 0; i < iterationBSP; i++)
			{
				...
				for (var j:int = 0; j < partitionNumber; j++)
				{
					...
					if (partitionWidth[j] < partitiionMin) directionBSP = 1;
					if (partitionHeight[j] < partitiionMin) directionBSP = 0;
					if ((partitionWidth[j] < partitiionMin) && (partitionHeight[j] < partitiionMin)) directionBSP = 2;
					//trace (directionBSP);
					if (directionBSP == 0)
					{
						positionBSP = (int(Math.random() * (splitStable)) - (splitStable)) + (partitionWidth[j]/2);
						//trace ("Vert:"+positionBSP);
